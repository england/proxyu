...
function s_doPlugins(s) {
	...
	if (typeof(s.eVar22) == 'undefined') {
		s.eVar22 = decodeURIComponent(window.location.href).toLowerCase();
		var decodedParams = ['email','email_address', 'username', 'password', 'password_verify'];
		for (var i = 0; i < decodedParams.length; i++) {
			if (s.Util.getQueryParam(decodedParams[i], s.eVar22)) {
				s.eVar22 = s.eVar22.replace(s.Util.getQueryParam(decodedParams[i], s.eVar22), '[' + decodedParams[i] + ']');
			}
		}
		// test if email in pathname
		if (s.eVar22.match(/email:(.*)\//)) {
			s.eVar22 = s.eVar22.replace(s.eVar22.match(/email:(.*)\//)[1],'[email]');
		}
	}
	...
	if (typeof(s.prop3) == 'undefined') {
		s.prop3 = decodeURIComponent(document.referrer).toLowerCase();
	}
